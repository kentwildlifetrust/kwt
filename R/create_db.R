...
create_db <- function(conn, db_name, app_role){
  ...
  # Set search path to include postgis schema
  DBI::dbExecute(conn, glue::glue_sql("ALTER DATABASE {DBI::dbQuoteIdentifier(conn,db_name)} SET search_path TO postgis, public;", .con=conn))
  # Print to console
