...
WHERE
  ...
  htmltools::tags$p(style = "font-size: 2rem;")
```
...
lapply(1:nrow(cols), function(i) {
  htmltools::div(
    ...
    if (cols$is_primary_key[i] == "YES") {
      htmltools::h4(sprintf("%s (PK)", kwt::to_title_case(cols$column_name[i])))
    } else {
      htmltools::h4(kwt::to_title_case(cols$column_name[i]))
    },
    htmltools::div(htmltools::strong("Name:"), cols$column_name[i]),
    htmltools::div(htmltools::strong("Data type:"), 
    ...
    if (cols$is_nullable[i] == "YES") {
      htmltools::p(htmltools::strong("Is nullable:"), "yes")
    } else {
      htmltools::p(htmltools::strong("Is nullable:"), "no")
    },
  ...
  htmltools::tagList(
    htmltools::tags$style(".card {border: 1px solid #ccc; padding: 1rem; margin: 1rem; border-radius: 5px; width: 20rem;} h4 {font-weight: 600;}"),
    .
  )
```
...
atts <- "SELECT {`att_cols`*} FROM {`params$table_schema`}.{`params$table_name`}" %>%
  glue::glue_sql(.con = conn) %>%
  DBI::dbGetQuery(conn, .) %>%
